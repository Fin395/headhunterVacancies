...
def test_vacancy_init(first_vacancy: Vacancy) -> None:
    ...
    assert (
        first_vacancy.requirement == "Занимать активную жизненную позицию, уметь активно танцевать и громко петь. "
        "Обладать навыками коммуникации, чтобы налаживать добрососедские отношения. Обладать системным мышлением..."
    )
...
def test_vacancy_str(first_vacancy: Vacancy) -> None:
    ...
    assert (
        str(first_vacancy)
        == "id вакансии: 93353083, наименование вакансии: Тестировщик комфорта квартир, зарплата: 450000, "
        "ссылка на вакансию: https://hh.ru/employer/3499705, требования: Занимать активную жизненную позицию, "
